...
for store in os.listdir(stores_path):
    if os.path.exists("%s/%s.git" % (stores_path, store)):
        backup_store_path = "%s/%s.git" % (backup_stores_path, store)
        if os.path.exists(backup_store_path):
            cmd = "git fetch %s/%s.git" % (stores_path, store)
            cwd = backup_store_path
        else:
            cmd = "git clone %s/%s.git ." % (stores_path, store)
            cwd = backup_stores_path
        # git clone/fetch to backup stores location
        subprocess.check_call(cmd, cwd=cwd, shell=True)
        # tarball of backup git repo
        subprocess.check_call("tar -czf %s.tgz %s.git" % (store, store), cwd=backup_stores_path, shell=True)
